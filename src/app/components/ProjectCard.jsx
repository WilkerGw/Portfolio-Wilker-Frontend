...
const ProjectCard = ({ project }) => {
  ...
  return (
    <Link href={`/projetos/${projectId}`} className={styles.card}>
      <motion.div
        className={styles.cardContent}
        initial="hidden"
        whileHover="visible"
      >
        {/* Imagem de Fundo (permanece igual) */}
        <Image
          src={imageUrl}
        ...
        {/* Título Visível Permanentemente */}
        <div className={styles.titleContainer}>
          <h3>{title}</h3>
        </div>

        {/* Overlay com as informações que aparecem no hover */}
        <motion.div
